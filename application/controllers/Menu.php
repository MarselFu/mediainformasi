...
class Menu extends CI_Controller
{
    ...
    public function edit_pengguna() {
        ...
        $data = [
            'nama_instansi' => $this->input->post('nik'),
            'email' => $this->input->post('email'),
            'alamat' => $this->input->post('alamat'),
            'username' => $this->input->post('username')
        ];

        if ($this->model->update_pengguna($id, $data)) {
            $this->session->set_flashdata('msg', 'diperbarui.');
        } else {
            $this->session->set_flashdata('msg', 'gagal diperbarui.');
        }
    ...
    public function cetak_laporan()
    {
        ...
        if (isset($_POST['btn-cek'])) {
            ...
            $this->dompdf->render();
            $this->dompdf->stream('Laporan Pengaduan Masyarakat Desa.pdf', ['Attachment' => true]); // false atau 0 untuk preview sebelum download
